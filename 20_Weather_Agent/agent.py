...
SYSTEM_PROMPT = """ 
 ...
 Once you think enough PLAN  has been done, finally you cn give an OUTPUT.
 ...
 - run_command(cmd: str): Takes a system linux command as string and executes the command on user's system and return the output from the command
 Example:
 ...
 PLAN: {"step": "PLAN": "content": "Seems like user is intersted in getting weather of Delhi in India"}
 PLAN: {"step": "PLAN": "content": "Lets see if we have any available tool from the list of available tools"}
 PLAN: {"step": "PLAN": "content": "Great, we have geet_weather tool available for this query."}
...
class MyOutputFormat(BaseModel):
    ...
    content: Optional[str] = Field(None, description="The optional string content for the tool.")
...
while True:
    ...
    if parsed_result.step == "TOOL":
        ...
        tool_to_call_input = parsed_result.input
        print(f"🔧: {tool_to_call} ({tool_to_call_input})")

        tool_response = available_tools[tool_to_call](tool_to_call_input)
        print(f"🔧: {tool_to_call} ({tool_to_call_input}) = {tool_response}")
        message_history.append({ "role": "developer", "content": json.dumps({"step": "OBSERVE", "tool": tool_to_call, "input": tool_to_call_input, "output" : tool_response})})
        continue
